...
for (int j = 0; j < height; j++) {
  ...
  for (int i = 0; i < width; i++) {
    ...
    if (n == maxIterations) {
      ...
      pixels[i+j*width] = color(243, 134, 250);
      
    } else if (n > 2 * maxIterations / 6 && n < 3 * maxIterations / 6) {
      pixels[i+j*width] = color(216, 48, 237);
      
    } else if (n > 3 * maxIterations / 6 && n < 4 * maxIterations / 6) {
      pixels[i+j*width] = color(170, 0, 215);
      
    } else if (n > 4 * maxIterations / 6 && n < 5 * maxIterations / 6) {
      pixels[i+j*width] = color(116, 0, 179);
      
    } else {
      pixels[i+j*width] = color(68, 0, 135);
    }
